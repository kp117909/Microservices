...
<nav class="bg-white dark:bg-gray-900 fixed w-full z-20 top-0 start-0 border-b border-gray-200 dark:border-gray-600">
  <div class="max-w-screen-xl flex flex-wrap items-center justify-between mx-auto p-4">
    <!-- Logo -->
      <a href="{{ auth()->check() ? route('page.dashboard') : route('page.welcome') }}" class="flex items-center space-x-3 rtl:space-x-reverse">
  ...
  <ul class="flex flex-col md:flex-row md:space-x-8 rtl:space-x-reverse p-4 md:p-0 mt-4 font-medium border border-gray-100 rounded-lg bg-gray-50 md:mt-0 md:border-0 md:bg-white dark:bg-gray-800 md:dark:bg-gray-900 dark:border-gray-700 w-full">
    ...
    @auth
      <li class="md:ml-auto mt-2 md:mt-0">
        <a href="{{ route('auth.logout') }}" class="block py-2 px-3 text-white bg-pink-900 hover:underline rounded md:bg-transparent md:text-blue-700 md:p-0 md:dark:text-blue-500">
          {{ __('Logout') }} <span aria-hidden="true">&rarr;</span>
        </a>
      </li>
    @else
      <li class="md:ml-auto mt-2 md:mt-0">
        <a href="{{ route('login') }}" class="block py-2 px-3 text-white bg-blue-700 hover:underline rounded md:bg-transparent md:text-blue-700 md:p-0 md:dark:text-blue-500">
